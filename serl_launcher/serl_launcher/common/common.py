...
class JaxRLTrainState(struct.PyTreeNode):
    ...
    def apply_gradients(self, *, grads: Any) -> "JaxRLTrainState":
        ...
        updates_acc = tree_util.tree_map(
    ...
    def apply_loss_fns(
        ...
        # compute gradients
        grads_and_aux = tree_util.tree_map(
        ...
        if has_aux:
            grads = tree_util.tree_map(lambda _, x: x[0], loss_fns, grads_and_aux)
            aux = tree_util.tree_map(lambda _, x: x[1], loss_fns, grads_and_aux)
